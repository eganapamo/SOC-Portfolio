...
Command Used:
sudo grep -aiE "sudo|chmod|wget|curl|bash|sh|useradd|adduser|usermod" /var/log/auth.log
...
Command Used:
sudo grep -aiE "sudo|chmod|wget|curl|bash|sh|useradd|adduser|usermod" /var/log/auth.log | grep -iv "COMMAND=/usr/bin/grep"
...
Command Used:
sudo grep -ai "COMMAND=" /var/log/auth.log | grep -aiE "sudo|chmod|wget|curl|bash|sh|useradd|adduser|usermod" | grep -iv "COMMAND=/usr/bin/grep" | awk -F "COMMAND=" '{print $2}'
